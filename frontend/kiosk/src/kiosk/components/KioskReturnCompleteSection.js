...
const KioskReturnCompleteSection = () => {
  ...
  const KioskNameURL = `http://127.0.0.1:8080/api/kiosk/home/kiosk-name?id=${id}`;

  // 오디오
  const [audio, setAudio] = useState(new Audio(audioFile));
  const [NoReturnAudio, setNoRentAudio] = useState(
    new Audio(NoReturnAudioFile)
  );
